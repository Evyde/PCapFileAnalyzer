...
class IPv4(IP):
    ...
    def __init__(self, link_obj):
        ...
        (__tos, __len, __id, __temp_off_flag, __ttl, __protocol, __chksum, __src, __dst) = struct.unpack('!BHHHBBHII',
                                                                                                         packet[1:20])
        __DF = __temp_off_flag >> 13 & 0b010
        __MF = __temp_off_flag >> 13 & 0b001
        __offset = __temp_off_flag & 0x1fff
        if __DF == 0 and __MF == 1:
            # fragment
            self.__fragmented = True
            if __offset == 0:
                self.__first = True
                from .LinkLayer import LinkLayer
                assert isinstance(self.get_link_obj(), LinkLayer)
                assert self.get_link_obj().get_physics()
        try:
    ...
    def add(self, ip: IP):
        ...
        self.fragments += 1
        self.used_time += (self.used_time - ip.timestamp) / self.fragments
