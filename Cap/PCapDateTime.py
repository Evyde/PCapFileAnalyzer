...
class PCapDateTime:
    def __init__(self, timestamp, calculated=False):
        ...
        if isinstance(timestamp, PCapDateTime):
            self.__source_ts = timestamp.get_source_ts()
        elif isinstance(timestamp, Decimal):
            self.__source_ts = timestamp
        else:
            self.__source_ts = Decimal(timestamp)
    ...
    def __truediv__(self, other):
        return PCapDateTime("{:.9f}".format(self.get_source_ts() / Decimal(other)), calculated=True)

    def __abs__(self):
        return PCapDateTime(abs(self.get_source_ts()), calculated=True)

    def sqrt(self):
        return PCapDateTime("{:.9f}".format(Decimal.sqrt(abs(self.get_source_ts() * pow(10, 3))) / pow(10, 3)),
                            calculated=True)
    ...
    def __str__(self):
        ...
        if self.__calculated:
            ...
            return (("{} s, ".format(Decimal(__int).to_integral_value()) if __int != Decimal(
                0) else "") + "{:.2f} ms".format(Decimal(__decimal) / pow(10, 6)))
        else:
            try:
                __int = int(__int)
            except:
            ...
            return datetime.datetime.fromtimestamp(__int).strftime(
                "%Y-%m-%d %H:%M:%S") + ", {:.2f} ms".format(Decimal(__decimal) / pow(10, 6))
