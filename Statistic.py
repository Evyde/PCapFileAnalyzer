...
class TCPConnection:
    ...
    last_size = -1
    open = False
    duration = PCapDateTime(Decimal(0.0), calculated=True)
    send_packets = 0
    recv_packets = 0
    packets = 0
    send_bytes = 0
    recv_bytes = 0
    max_rtt = PCapDateTime(Decimal(0.0), calculated=True)
    min_rtt = PCapDateTime(Decimal(99999999.0), calculated=True)
    mean_rtt = PCapDateTime(Decimal(0.0), calculated=True)
    last_send_index = -1
    last_recv_index = -1
    max_window = 0
    ...
    def __init__(self, init_packet: TCP, index):
        # Trust init_packet
        self.src = init_packet.get_header().src
        self.src_port = init_packet.get_header().src_port
        self.dst = init_packet.get_header().dst
        self.dst_port = init_packet.get_header().dst_port
        self.start = init_packet.get_header().ts
        self.max_window = init_packet.get_header().window
        self.min_window = init_packet.get_header().window
        self.mean_window = init_packet.get_header().window
        self.last_time = init_packet.get_header().ts
        self.last_ack = init_packet.get_header().ack_num
        self.send_packets = 1
        self.packets = 1
        self.flags += self.generate_flag(init_packet)
        self.last_send_seq = init_packet.get_header().seq
        self.last_send_index = index

    def generate_flag(self, pkt: TCP):
        __rtn_flags = ""
        if pkt.get_header().syn == 1:
            __rtn_flags += "S"
        elif pkt.get_header().fin == 1:
            __rtn_flags += "F"
        if pkt.get_header().rst == 1:
            __rtn_flags += "R"
        return __rtn_flags

    def add_send(self, pkt: TCP, index):
        self.last_ack = pkt.get_header().ack_num
        self.last_size = pkt.get_header().payload_size
        self.flags += self.generate_flag(pkt)
        self.send_packets += 1
        self.packets += 1
        self.send_bytes += pkt.get_header().payload_size
        if self.last_status != "send":
            # calculate RTT
            self.rtt_num += 1
            __temp_rtt = abs(pkt.get_header().ts - self.last_time)
        ...
        self.last_time = pkt.get_header().ts
        # calculate window
        self.mean_window = abs(pkt.get_header().window - self.mean_window) / self.packets
        if pkt.get_header().window >= self.max_window:
            self.max_window = pkt.get_header().window
        if pkt.get_header().window <= self.min_window:
            self.min_window = pkt.get_header().window
        self.last_send_seq = pkt.get_header().seq
        self.last_send_index = index
    ...
    def add_recv(self, pkt: TCP, index):
        ...
        self.recv_packets += 1
        self.packets += 1
        self.recv_bytes += pkt.get_header().payload_size
        ...
        if pkt.get_header().window <= self.min_window:
            self.min_window = pkt.get_header().window
        self.last_recv_seq = pkt.get_header().seq
        self.last_recv_index = index

    def close(self, close_pkt: TCP):
        # Trust close packet
        self.complete = True
        self.end = close_pkt.get_header().ts
        self.duration = self.end - self.start
        if self.src == close_pkt.get_header().src:
            # close by client
...
class Statistic:
    ...
    def __init__(self, packets, instant=False):
        ...
        for packet in packets:
            if not instant:
                __p.next(1)
            try:
                if packet.link_obj.get_ip_obj() is None or packet.link_obj.get_tcp_obj() is None:
                    continue
            except:
                continue

            __temp_list = self.__tcps.get(packet.link_obj.get_tcp_obj().__hash__(), [])
            __temp_list.append(packet.link_obj.get_tcp_obj())
            self.__tcps[packet.link_obj.get_tcp_obj().__hash__()] = __temp_list
        ...
        for i in self.__tcps.keys():
            self.__tcps[i] = sorted(self.__tcps[i], key=lambda t: t.get_header().ts)
        ...
        for i in self.__tcps.keys():
            for tcp in range(len(self.__tcps[i])):
                ...
                if self.__tcps[i][tcp].used:
                    continue
                if self.__tcps[i][tcp].get_header().syn == 1 and self.__tcps[i][tcp].get_header().ack == 0 and \
                        self.__tcps[i][tcp].get_header().fin == 0:
                    ...
                    if self.__tcps.get(__dst_hash, None):
                        for dst_tcp in range(len(self.__tcps[__dst_hash])):
                            if self.__tcps[__dst_hash][dst_tcp].used:
                                continue
                            if self.__tcps[__dst_hash][dst_tcp].get_header().ack_num == (
                                    self.__tcps[i][tcp].get_header().seq + 1):
                                if self.__tcps[__dst_hash][dst_tcp].get_header().syn == 1 and \
                                        self.__tcps[__dst_hash][dst_tcp].get_header().ack == 1:
                                    ...
                                    for tcp_1 in range(len(self.__tcps[i])):
                                        if self.__tcps[i][tcp_1].get_header().ack_num == (
                                                self.__tcps[__dst_hash][dst_tcp].get_header().seq + 1):
                                            if self.__tcps[i][tcp_1].get_header().ack == 1:
                                                # found valid client
                                                # open but not close
                                                temp_tcp_connection.open = True
                                                self.__tcps[i][tcp_1].used = True
                                                temp_tcp_connection.add_send(self.__tcps[i][tcp_1], tcp_1)
                                            if self.__tcps[i][tcp_1].get_header().rst == 1:
                                                self.__tcps[i][tcp_1].used = True
                                                temp_tcp_connection.add_recv(self.__tcps[i][tcp_1], tcp_1)
                                                break
                                if self.__tcps[__dst_hash][dst_tcp].get_header().rst == 1:
                                    self.__tcps[__dst_hash][dst_tcp].used = True
                                    temp_tcp_connection.add_recv(self.__tcps[__dst_hash][dst_tcp], dst_tcp)
                                    break
                    ...
                    if temp_tcp_connection.open:
                        ...
                        while not __no_more:
                            ...
                            for normal_send in range(len(self.__tcps[i])):
                                ...
                                if self.__tcps[i][normal_send].get_header().seq == \
                                    ...
                                    for more_send in range(len(self.__tcps[i])):
                                        if self.__tcps[i][more_send].used:
                                            continue
                                        if self.__tcps[i][more_send].get_header().seq == \
                                                temp_tcp_connection.last_ack + \
                                    ...
                                    for recv in range(len(self.__tcps[__dst_hash])):
                                        if self.__tcps[__dst_hash][recv].used:
                                            continue
                                        if self.__tcps[__dst_hash][recv].get_header().seq == \
                                                temp_tcp_connection.last_ack + \
                                                temp_tcp_connection.last_size:
                                            # found more
                            ...
                            if self.__tcps[i][client_tcp].get_header().fin == 1:
                                ...
                                for server_tcp in range(len(self.__tcps[__dst_hash])):
                                    if self.__tcps[__dst_hash][server_tcp].used:
                                        continue
                                    if self.__tcps[__dst_hash][server_tcp].get_header().fin == 0 and \
                                            self.__tcps[__dst_hash][server_tcp].get_header().ack_num == \
                                            self.__tcps[i][client_tcp].get_header().seq + 1:
                                        ...
                                        if server_tcp + 1 >= len(self.__tcps[__dst_hash]):
                                            break
                                        if self.__tcps[__dst_hash][server_tcp + 1].get_header().fin == 1:
                                            ...
                                            for client_ack_tcp in range(len(self.__tcps[i])):
                                                if self.__tcps[i][client_ack_tcp].used:
                                                    continue
                                                if self.__tcps[i][client_ack_tcp].get_header().fin == 1 \
                                                        and self.__tcps[i][
                                                    client_ack_tcp].get_header().ack_num == \
                                                    ...
                                                    # check if rst
                                                if self.__tcps[i][client_ack_tcp].get_header().seq == \
                                                        temp_tcp_connection.last_recv_seq + 1 and \
                                                        self.__tcps[i][
                                                            client_ack_tcp].get_header().rst == 1:
                                                    # RST send by client
                                                    ...
                                                    break
                                            # check if rst
                                            if self.__tcps[__dst_hash][server_tcp].get_header().seq == \
                                                    temp_tcp_connection.last_recv_seq + 1 and \
                                                    self.__tcps[__dst_hash][
                                                        server_tcp].get_header().rst == 1:
                                                # RST send by client
                                    ...
                                    if self.__tcps[__dst_hash][server_tcp].get_header().seq == \
                                            temp_tcp_connection.last_recv_seq + 1 and \
                                            self.__tcps[__dst_hash][server_tcp].get_header().rst == 1:
                                        # RST send by client
                        ...
                        if not __rst and not temp_tcp_connection.complete:
                            for serv_tcp in range(len(self.__tcps[__dst_hash])):
                                ...
                                if self.__tcps[__dst_hash][serv_tcp].get_header().fin == 1:
                                    self.__tcps[__dst_hash][serv_tcp].used = True
                                    temp_tcp_connection.add_send(self.__tcps[__dst_hash][serv_tcp], serv_tcp)
                                    # check if rst
                                    if self.__tcps[__dst_hash][serv_tcp].get_header().seq == \
                                            temp_tcp_connection.last_send_seq + 1 and \
                                            self.__tcps[__dst_hash][serv_tcp].get_header().rst == 1:
                                        # RST send by client
                                        temp_tcp_connection.add_send(self.__tcps[__dst_hash][serv_tcp], serv_tcp)
                                        __rst = True
                                        break
                                    ...
                                    for clien_tcp in range(len(self.__tcps[i])):
                                        if self.__tcps[i][clien_tcp].used:
                                            continue
                                        if self.__tcps[i][clien_tcp].get_header().fin == 0 and \
                                                self.__tcps[i][clien_tcp].get_header().ack_num == \
                                                self.__tcps[__dst_hash][serv_tcp].get_header().seq + 1:
                                            ...
                                            if clien_tcp + 1 >= len(self.__tcps[i]):
                                                break
                                            if self.__tcps[i][clien_tcp + 1].get_header().fin == 1:
                                                ...
                                                for client_ack_tcp in range(len(self.__tcps[__dst_hash])):
                                                    if self.__tcps[__dst_hash][client_ack_tcp].used:
                                                        continue
                                                    if self.__tcps[__dst_hash][client_ack_tcp].get_header().fin == 1 \
                                                        ...
                                                        # check if rst
                                                    if self.__tcps[__dst_hash][client_ack_tcp].get_header().seq == \
                                                            temp_tcp_connection.last_recv_seq + 1 and \
                                                            self.__tcps[__dst_hash][
                                                                client_ack_tcp].get_header().rst == 1:
                                                        # RST send by client
                                                        ...
                                                        break
                                                # check if rst
                                                if self.__tcps[i][clien_tcp].get_header().seq == \
                                                        temp_tcp_connection.last_recv_seq + 1 and \
                                                        self.__tcps[i][
                                                            clien_tcp].get_header().rst == 1:
                                                    # RST send by client
                                        ...
                                        if self.__tcps[i][clien_tcp].get_header().seq == \
                                                temp_tcp_connection.last_recv_seq + 1 and \
                                                self.__tcps[i][clien_tcp].get_header().rst == 1:
                                            # RST send by client
                                            temp_tcp_connection.add_send(self.__tcps[i][clien_tcp], clien_tcp)
                                            __rst = True
                                            break
    ...
    def print(self):
        ...
        max_duration = PCapDateTime(Decimal(0), calculated=True)
        min_duration = PCapDateTime(Decimal(99999), calculated=True)
        mean_duration = PCapDateTime(Decimal(0), calculated=True)
        min_rtt = PCapDateTime(Decimal(99999), calculated=True)
        max_rtt = PCapDateTime(Decimal(0), calculated=True)
        mean_rtt = PCapDateTime(Decimal(0), calculated=True)
        max_pkt = 0
        min_pkt = 0
        mean_pkt = 0
        min_window = 0
        ...
        for c in self.__tcp_connections:
            ...
            if c.complete:
                ...
                print("Start Time: {}".format("{}".format(str(c.start))))
                print("End Time: {}".format("{}".format(str(c.end))))

                print("Duration: {}".format("{}".format(str(c.duration))))
        ...
        print("Minimum time duration: {}".format("{}".format(str(min_duration))))
        print("Mean time duration: {}".format("{}".format(str(mean_duration))))
        print("Maximum time duration: {}".format("{}".format(str(max_duration))))

        print()
        print("Minimum RTT value: {}".format("{}".format(str(min_rtt))))
        print("Mean RTT value: {}".format("{}".format(str(mean_rtt))))
        print("Maximum RTT value: {}".format("{}".format(str(max_rtt))))

        print()
        print("Minimum number of packets including both send/received: {}".format(min_pkt))
        print("Mean number of packets including both send/received: {:.2f}".format(mean_pkt))
        ...
        print("Mean receive window size including both send/received: {:.2f}".format(mean_window))
